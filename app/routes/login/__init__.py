...
from schemas.responses_model.common import EnumErrors, ResponseBase, ErrorResponse, DetailErrorObj,EnumMsg
...
async def verify(email:str,passworld:str):
    ...
    try:
        results = await LoginQueries().buscar_usuario(email=email,passworld=passworld)
        if not results:
            raise ErrorResponse(
